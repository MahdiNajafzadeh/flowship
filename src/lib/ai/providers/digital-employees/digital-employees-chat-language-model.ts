...
import type { DigitalEmployeesChatSettings, DigitalEmployeesChatConfig } from "./digital-employees-types";
...
export class DigitalEmployeesChatLanguageModel implements LanguageModelV4 {
    ...
    constructor(modelId: string, settings: DigitalEmployeesChatSettings, config: DigitalEmployeesChatConfig) {
    ...
    async doGenerate(options: LanguageModelV4CallOptions): Promise<LanguageModelV4GenerateResult> {
        throw new APICallError({
            message: "The Digital Employees provider only supports streaming. Use streamText instead of generateText.",
    ...
    private buildRequestBody(options: LanguageModelV4CallOptions): Record<string, unknown> {
    ...
    private createStream(response: Response, warnings: SharedV4Warning[]): ReadableStream<LanguageModelV4StreamPart> {
                    ...
                    while (true) {
                        ...
                        for (const line of lines) {
                            ...
                            if (parsed.agentic_event) {
                                const ae =
                                    typeof parsed.agentic_event === "object" && parsed.agentic_event !== null
                                        ? (parsed.agentic_event as Record<string, unknown>)
                                        : parsed;

                                const toolCallId = (ae.tool_call_id ?? ae.toolCallId ?? "") as string;
                                const hasResult =
                                    ae.result !== undefined || ae.output !== undefined || ae.data !== undefined;

                                if (toolCallId && hasResult) {
                                    const toolName = (ae.tool_name ?? ae.toolName ?? "unknown") as string;
                                    const resultValue = ae.result ?? ae.output ?? ae.data ?? {};

                                    const toolResult: LanguageModelV4ToolResult = {
                                        type: "tool-result",
                                        toolCallId,
                                        toolName,
                                        result: resultValue as NonNullable<JSONValue>,
                                        isError: false,
                                    };
                            ...
                            if (parsed.created && !responseTimestamp) {
                                responseTimestamp = new Date((parsed.created as number) * 1000);
                            }
                            ...
                            if (delta.tool_calls) {
                                ...
                                for (const tc of toolCalls) {
                                    ...
                                    const state = toolCallMap.get(idx) as unknown as ToolCallState;
                            ...
                            if (finishReason && finishReason !== "tool_calls" && finishReason !== "null") {
                                if (hasStartedReasoning) {
                                    controller.enqueue({
                                        type: "reasoning-end",
                                        id: reasoningId,
...
function stringifyToolOutput(output: LanguageModelV4ToolResultPart["output"]): string {
    switch (output.type) {
        case "text":
            return output.value;
        case "json":
...
function mapFinishReason(reason: string): "stop" | "length" | "content-filter" | "tool-calls" | "error" | "other" {
    switch (reason) {
        case "stop":
            return "stop";
        case "length":
